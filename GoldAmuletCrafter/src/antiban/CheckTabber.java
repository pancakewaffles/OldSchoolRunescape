...
public class CheckTabber {
	...
	final int SKILLTAB_X = 640;
	final int SKILLTAB_Y = 345;
	...
	private void checkSkills() {
		...
		if(script.getTabs().getOpen() == Tab.SKILLS) {
			...
			int realisticskilltab_x = (int) (Math.floor(Math.random()*stddev) + SKILLTAB_X);
			int realisticskilltab_y = (int) (Math.floor(Math.random()*stddev) + SKILLTAB_Y);
