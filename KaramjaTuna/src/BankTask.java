...
public class BankTask extends Task {
    ...
    public int execute() {
        ...
        if (script.getDepositBox().isOpen()) {
            if (script.getInventory().contains("Raw tuna")) {
                script.depositBox.depositAllExcept("Harpoon", "Coins");
                new ConditionalSleep(3000,5000) {
                    @Override
                    public boolean condition() throws InterruptedException {
                        return !script.inventory.contains("Raw tuna");
